...
class WebcamColorExtractor:
    ...
    def draw_bottom_branding(self, frame: np.ndarray) -> np.ndarray:
        ...
        cv2.putText(frame, "Controls: 'q'/'x'/ESC to quit | 'c' to capture", (width - 350, height - 40), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 255), 1, cv2.LINE_AA)
    ...
    def run(self):
        ...
        print("⌨️  Controls: 'q'/'x'/ESC to quit | 'c' to force capture")
...
def main():
    """Main function"""
    try:
        extractor = WebcamColorExtractor(n_colors=5)
        extractor.run()
